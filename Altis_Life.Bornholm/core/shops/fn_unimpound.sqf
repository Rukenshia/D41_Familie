...
if(_veh distance impound_obj < 50) then
{
	...
	{
		_price = _price + 200;
		if(_price > D41_atmGeld) exitWith {hint "You do not have enough money on you or in your bank to get your car back."};
	...
	{
		_veh setPos [_pos select 0, _pos select 1, 1.5];
	}
	else
	{
		_veh setPos _pos;
