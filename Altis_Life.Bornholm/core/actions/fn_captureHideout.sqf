...
if ({side _x == civilian} count playableUnits < 10) exitWith {hint "Es müssen mindestens 10 Zivilisten Online sein, um Gangverstecke einnehmen zu können."};
...
_cP = 0.01;
[[[0,1],format["%1 und seine Gang: %2 versuchen die Kontrolle über ein Gangversteck zu übernehmen!",name player,(group player) getVariable "gang_name" ]],"life_fnc_broadcast",true,false] call life_fnc_MP;
...
if((player distance (getMarkerPos "Ganghideout01")) < 100)	then { "Ganghideout01" setMarkerColor "ColorBlack"; "Ganghideout01" setMarkerText format["Versteck Bravo erobert von %1",(group player) getVariable "gang_name"];};
if((player distance (getMarkerPos "Ganghideout02")) < 100)	then { "Ganghideout02" setMarkerColor "ColorBlack"; "Ganghideout02" setMarkerText format["Versteck Alpha erobert von %1",(group player) getVariable "gang_name"];};
if((player distance (getMarkerPos "Ganghideout03")) < 100)	then { "Ganghideout03" setMarkerColor "ColorBlack"; "Ganghideout03" setMarkerText format["Versteck Charlie erobert von %1",(group player) getVariable "gang_name"];};